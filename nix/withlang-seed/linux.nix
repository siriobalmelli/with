...
stdenv.mkDerivation (finalAttrs: {
  ...
  # Patches frozen v0.15.1 emitted C until a seed includes these source-level overrides.
  patches = [ ./patches/linux-seed-link-paths.patch ];

  postPatch = ''
    substituteInPlace src/with_compiler.c \
      --replace-fail '@WITH_LINUX_DYNAMIC_LINKER@' '${dynamicLinker}' \
      --replace-fail '@WITH_LINUX_CRT_DIR@' '${libcDir}' \
      --replace-fail '@WITH_LINUX_GCC_DIR@' '${gccDir}' \
      --replace-fail '@WITH_LINUX_SYSLIB_DIR@' '${gccLibDir}'
  '';
  ...
  buildPhase = ''
    ...
    "${withlang-llvm}/bin/clang" "''${cflags[@]}" \
      -include runtime/wl_decls.h \
  ...
  installPhase = ''
    ...
    makeWrapper ${withlang-llvm}/bin/clang "$out/bin/with-seed-cc" \
