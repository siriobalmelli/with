...
impl Parser:
    ...
    fn record_top_level_stmt(stmt: NodeId) -> Unit:
        if stmt != 0:
            self.top_level_stmts.push(stmt as i32)

    mut fn emit_explicit_main_conflict(stmt: NodeId):
    ...
    mut fn expect(expected: i32) -> i32:
    ...
    mut fn expect_ident_or_keyword() -> i32:
        let t = self.peek()
...
impl Parser:
    ...
    mut fn parse_effect_name_bit() -> i32:
        if self.peek() != TokenKind.TK_IDENT:
    ...
    fn default_comptime_capability_binding(type_node: NodeId) -> i32:
        if type_node == 0:
            return 0
    ...
    mut fn parse_comptime_with_decl(start: i32, outer_pub: i32) -> NodeId:
    ...
    mut fn parse_comptime_decl_block():
    ...
    mut fn parse_extern_decl(start: i32) -> NodeId:
    ...
    mut fn parse_type_decl(is_pub: i32, start: i32) -> NodeId:
    ...
    fn queue_synthetic_copy_impl(type_name: i32, tp_start: i32, tp_count: i32, start: i32, end: i32) -> Unit:
    ...
    mut fn parse_enum_decl(is_pub: i32, start: i32) -> NodeId:
    ...
    mut fn emit_legacy_enum_decl_error():
        self.emit_error("use 'enum' for enum declarations")

    mut fn emit_legacy_struct_decl_error():
        self.emit_error("drop '=' in struct type declarations")

    mut fn parse_enum_named_decl(start: i32, name: i32, is_pub: i32, tp_start: i32, tp_count: i32, is_ephemeral: i32) -> NodeId:
        var repr_type_node: NodeId = 0 as NodeId
    ...
    mut fn parse_use_decl(start: i32) -> NodeId:
...
impl Parser:
    ...
    mut fn parse_impl_block(vis: i32):
        ...
        if is_extend != 0:
            self.pool.mark_extend_impl(impl_node)
...
impl Parser:
    ...
    mut fn parse_int_literal() -> NodeId:
        let start = self.current_start()
        let end = self.current_end()
    ...
    mut fn parse_float_literal() -> NodeId:
        let start = self.current_start()
        let end = self.current_end()
        let text = self.source.slice(start as i64, end as i64)
        if numeric_literal_suffix_is_separated(text):
    ...
    mut fn parse_comptime_error_expr() -> NodeId:
...
impl Parser:
    mut fn parse_regex_literal() -> NodeId:
        let start = self.current_start()
        let end = self.current_end()
        let text = self.source.slice(start as i64, end as i64)
    ...
    fn build_regex_match_call(lhs: NodeId, rhs: NodeId, start: i32, end: i32) -> NodeId:
...
fn offset_interpolated_expr_spans(pool: AstPool, first_node: i32, delta: i32):
    if delta == 0:
        return
    var ni = first_node
    while ni < pool.node_count():
        let node = ni as NodeId
        pool.set_start(node, pool.get_start(node) + delta)
        pool.set_end(node, pool.get_end(node) + delta)
        ni = ni + 1
...
impl Parser:
    mut fn parse_ident_or_call() -> NodeId:
        let start = self.current_start()
        let end = self.current_end()
        let sym = self.intern_current()
        self.advance()
    ...
    mut fn parse_postfix(lhs_in: i32) -> NodeId:
    ...
    fn maybe_wrap_implicit_it(expr: i32) -> NodeId:
    ...
    mut fn parse_assert_matches(callee: i32) -> NodeId:
    ...
    mut fn parse_call(callee: i32) -> NodeId:
        self.advance()  // consume (
    ...
    mut fn parse_dot(lhs: i32) -> NodeId:
        self.advance()  // consume .
        // .await
    ...
    fn is_positional_struct_literal() -> i32:
        if self.peek() == TokenKind.TK_R_BRACE:
            return 0
        if self.peek() != TokenKind.TK_IDENT:
            return 1
    ...
    mut fn parse_struct_literal(lhs: i32) -> NodeId:
    ...
    mut fn parse_single_index_spec() -> NodeId:
        let start = self.current_start()
        // Ellipsis: ...
        if self.peek() == TokenKind.TK_DOT_DOT_DOT:
            self.advance()
    ...
    mut fn parse_variant_shorthand() -> NodeId:
        let start = self.current_start()
        let end = self.current_end()
        let text = self.source.slice((start + 1) as i64, end as i64)
        let sym = self.intern.intern(text)
    ...
    mut fn parse_unary_negate() -> NodeId:
        let start = self.current_start()
        self.advance()
        if self.peek() == TokenKind.TK_INT_LIT:
            let end = self.current_end()
    ...
    mut fn parse_deref_expr() -> NodeId:
        let start = self.current_start()
        self.advance()
        let operand = self.parse_primary()
...
impl Parser:
    mut fn parse_if_else_body(chain_col: i32, chain_is_stmt: i32) -> NodeId:
        if self.peek() == TokenKind.TK_KW_IF:
            return self.parse_if_chain_arm(chain_col, chain_is_stmt)
        self.parse_body()

    mut fn parse_if_expr() -> NodeId:
    ...
    mut fn parse_if_let(start: i32, chain_col: i32, chain_is_stmt: i32) -> NodeId:
    ...
    mut fn parse_scope_expr() -> NodeId:
        let start = self.current_start()
        self.advance()
    ...
    mut fn parse_async_expr() -> NodeId:
        let start = self.current_start()
        self.advance()
    ...
    mut fn parse_select_await() -> NodeId:
        let start = self.current_start()
    ...
    mut fn parse_while(label: i32) -> NodeId:
        let start = self.current_start()
        self.advance()
        self.skip_newlines()
    ...
    mut fn parse_do_while(label: i32, stmt_start: i32) -> NodeId:
        let start = self.current_start()
    ...
    mut fn parse_while_let(start: i32) -> NodeId:
        self.advance()  // consume let
        let pat = self.parse_pattern()
        if self.expect(TokenKind.TK_EQ) == 0:
            return self.poisoned_expr()
    ...
    mut fn parse_loop(label: i32) -> NodeId:
        let start = self.current_start()
        self.advance()
        let body = self.parse_body()
    ...
    mut fn parse_for(label: i32) -> NodeId:
        let start = self.current_start()
        self.advance()
        self.skip_newlines()
        var binding = 0
    ...
    fn label_declaration_starts_statement(start: i32) -> i32:
    ...
    mut fn parse_labeled_statement() -> NodeId:
        let start = self.current_start()
    ...
    mut fn parse_match_expr() -> NodeId:
        let start = self.current_start()
        self.advance()
        self.skip_newlines()
        let saved_suppress_brace = self.suppress_brace
    ...
    mut fn parse_match_arms() -> i32:
        ...
        while self.peek() != TokenKind.TK_EOF:
            ...
            var body: NodeId = 0 as NodeId
            if self.peek() == TokenKind.TK_L_BRACE:
                // A brace after `=>` may be either a normal block or a record
                // update (`{ base with field }`). parse_brace_expr distinguishes
                // them; consuming `{` here made record updates unreachable.
                body = self.parse_expr()
            else:
    ...
    mut fn parse_const_binding() -> NodeId:
        let start = self.current_start()
        self.advance()  // consume 'const'
        let name_sym = self.expect_ident()
        if name_sym == 0:
    ...
    mut fn parse_brace_expr() -> NodeId:
        let start = self.current_start()
        self.advance()  // consume {
        self.skip_separators()
    ...
    mut fn parse_array_literal() -> NodeId:
        let start = self.current_start()
        self.advance()  // consume [
        self.skip_newlines()
        var elems: Vec[i32] = Vec.new()
    ...
    mut fn parse_fat_arrow_single() -> NodeId:
        let start = self.current_start()
    ...
    mut fn parse_fn_type_param_type() -> NodeId:
    ...
    mut fn parse_fn_type_node(start: i32, kind: i32) -> NodeId:
        self.advance()
        self.expect(TokenKind.TK_L_PAREN)
        self.skip_newlines()
    ...
    // `synth_recv_type`, and return the receiver mode plus SYNTH_RECEIVER bits. The
    // type is a literal `Self` node so it routes through the existing handling.
    mut fn build_synth_receiver(mode: i32) -> i32:
        ...
        if mode == 1:
            ...
            return FN_PARAM_FLAG_REF_SELF | FN_PARAM_FLAG_SYNTH_RECEIVER
        self.synth_recv_type = self_ty as i32
        if mode == 2:
            return FN_PARAM_FLAG_MUT_SELF | FN_PARAM_FLAG_SYNTH_RECEIVER
        FN_PARAM_FLAG_MOVE_SELF | FN_PARAM_FLAG_SYNTH_RECEIVER
    ...
    mut fn emit_pending_comptime_with_params() -> i32:
        let pattern_start = self.pool.fn_param_patterns_len()
    ...
    mut fn parse_one_type_param() -> i32:
    ...
    mut fn parse_optional_where_clause():
