...
fn reduce_then_relu:
    ...
    let bindings2 = bindings_from(move entries2)
    let event2 = dispatch(stream2, prog2, bindings2).unwrap()
    assert(event_is_done(event2))
    assert(read_i32(out2, 0) == 0)
    assert(read_i32(out2, 1) == 0)
    assert(read_i32(out2, 2) == 0)
    assert(read_i32(out2, 3) == 2)
    assert(read_i32(out2, 4) == 5)
